...
int gateway_json_encode_broadcast(char *out, size_t size, const gateway_broadcast_message_t *m, const gateway_config_t *c)
{
    ...
    snprintf(mac,sizeof(mac),"%02X:%02X:%02X:%02X:%02X:%02X",m->device.report.address[5],m->device.report.address[4],m->device.report.address[3],m->device.report.address[2],m->device.report.address[1],m->device.report.address[0]);
    if (!append(out,size,&used,"{") || !key_string(out,size,&used,"message_type","broadcast",true) ||
        ...
        !key_string(out,size,&used,"device_id",mac,true) || !key_string(out,size,&used,"device_name",m->device.report.name,true) ||
        !key_string(out,size,&used,"mac",mac,true) || !append(out,size,&used,"\"address_type\":%u,",m->device.report.address_type) ||
        !append(out,size,&used,"\"rssi\":%d,\"time_synced\":%s,\"event_uptime_s\":%lu,",m->device.report.rssi,m->time_synced?"true":"false",(unsigned long)m->event_uptime_s) ||
...
int gateway_json_encode_health(char *out, size_t size, const char *event_id, const gateway_config_t *c, uint32_t uptime_s, const char *wifi, const char *mqtt, const char *sntp, bool sd, uint32_t outbox)
{
    size_t used=0;
    if (!out || !event_id || !c) return -1;
    if (!append(out,size,&used,"{") || !key_string(out,size,&used,"message_type","gateway_health",true) || !key_string(out,size,&used,"event_id",event_id,true) || !key_string(out,size,&used,"gateway_id",c->gateway_id,true) || !append(out,size,&used,"\"uptime_s\":%lu,",(unsigned long)uptime_s) || !key_string(out,size,&used,"wifi",wifi,true) || !key_string(out,size,&used,"mqtt",mqtt,true) || !key_string(out,size,&used,"sntp",sntp,true) || !append(out,size,&used,"\"sd_ready\":%s,\"outbox_messages\":%lu}",sd?"true":"false",(unsigned long)outbox)) return -1;
    return (int)used;
}
