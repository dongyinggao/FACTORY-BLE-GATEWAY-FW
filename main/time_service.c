...
static uint32_t sync_wall_ms;

static void time_sync_callback(struct timeval *timeval)
{
    (void)timeval;
    sync_wall_ms = (uint32_t)(esp_timer_get_time() / 1000LL);
    time_synced = true;
...
bool time_service_format_wall_ms(uint32_t wall_ms, char *output, size_t output_size)
{
    ...
    int64_t event_ms;
    uint32_t current_wall_ms;
    if (!time_synced || wall_ms < sync_wall_ms || output == NULL || output_size == 0) return false;
    gettimeofday(&now, NULL);
    current_wall_ms = (uint32_t)(esp_timer_get_time() / 1000LL);
    event_ms = (int64_t)now.tv_sec * 1000LL + now.tv_usec / 1000LL - (uint32_t)(current_wall_ms - wall_ms);
