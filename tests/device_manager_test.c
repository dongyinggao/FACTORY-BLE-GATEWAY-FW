...
static void test_mac_is_identity(void)
{
    ...
    assert(device_registry_process_report(&registry, &second, 2, &index) == DEVICE_REGISTRY_UPDATED);
    assert(index == 0);
}
...
static void test_pause_freezes_timeout(void)
{
    ...
    assert(device_registry_process_report(&registry, &report, 0, &index) == DEVICE_REGISTRY_ADDED);
