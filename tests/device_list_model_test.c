...
static managed_device_t device_for(const char *name, uint8_t address_last, bool online, uint32_t last_seen_ms)
{
...
static void test_add_update_and_counts(void)
{
    ...
    managed_device_t first = device_for("ICD001", 1, true, 10);
    ...
    assert(device_list_model_online_count(&model) == 1);
    ...
    assert(device_list_model_count(&model) == 1);
    assert(device_list_model_online_count(&model) == 0);
...
{
    device_list_model_t model = {0};
    managed_device_t offline_new = device_for("ICD001", 1, false, 300);
    managed_device_t online_old = device_for("ICM_A01", 2, true, 100);
    managed_device_t online_new = device_for("ICD002", 3, true, 200);
    ...
    assert(strcmp(device_list_model_get_ranked(&model, 0)->report.name, "ICD002") == 0);
    assert(strcmp(device_list_model_get_ranked(&model, 1)->report.name, "ICM_A01") == 0);
    assert(strcmp(device_list_model_get_ranked(&model, 2)->report.name, "ICD001") == 0);
...
{
    device_list_model_t model = {0};
    managed_device_t extra = device_for("ICM_A02", 20, true, 100);

    for (uint8_t address = 0; address < DEVICE_MANAGER_MAX_DEVICES; ++address) {
        managed_device_t device = device_for("ICM_A01", address, true, 100);
